...
export const useGroupChatOnline = (userid: string) => {
  ...
  useEffect(() => {
    // update supabase data in realTime
    const channel = supabaseClient.channel("tracking")

    channel
      .on("presence", { event: "sync" }, () => {
        const state: any = channel.presenceState()
        console.log("supabase channel",state)
      ...
      // subscribe to listen to database changes.
...
export const useSearch = (search: "GROUPS" | "POSTS") => {
  ...
  useEffect(() => {
    console.log(":K:K:K:K", query)
    const delayInputTimeoutId = setTimeout(() => {
      setDebounce(query)
    }, 1000)
    console.log(":P:P:P:P", delayInputTimeoutId)
  ...
  useEffect(() => {
    ...
    if (!debounce) dispatch(onClearSearch())
      console.log("@O@L@L@L@L", debounce)
    return () => {
      debounce
    }
  }, [debounce])
...
export const useGroupSettings = (groupid: string) => {
  ...
  //  here change json to html to display in screen
  // cz html is faster while json is better fro editting staff 
  ...
  const onSetDescriptions = () => {
    ...
    // assign the setValue string that match with the GroupSettingsSchema not random
...
export const useGroupList = (query: string) => {
  ...
  // it let us clear the previous data  and ready for the next data of infinite scroll
  // note that: we dont dispatch the next scrolled fetched data
  useLayoutEffect(() => {
